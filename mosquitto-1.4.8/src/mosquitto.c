...
//=========statistics========
extern int g_flag_epoll;
extern unsigned long max_memcount;
extern unsigned long memcount;
extern unsigned long free_memcount;
extern unsigned long g_epoll_register_fd;
extern unsigned long g_epoll_unregister_fd;
extern unsigned long g_epoll_monitor_fd;
extern unsigned long g_epoll_expired_fd;

extern unsigned long g_epoll_sock_connections;
extern unsigned long g_epoll_mqtt_connections;
extern unsigned long g_epoll_publish_num;
extern unsigned long g_epoll_subscribe_num;
extern unsigned long g_epoll_unsubscribe_num;
...
void handle_sigusr1(int signal)
{
	...
	flag_db_backup = true;
#endif
    g_epoll_sock_connections = 0;
    g_epoll_mqtt_connections = 0;
    g_epoll_publish_num = 0;
    g_epoll_subscribe_num = 0;
    g_epoll_unsubscribe_num = 0;
}
...
void handle_sigusr2(int signal)
{
    ...
    printf("===================================================\n");
    printf("mosquttio max malloc memory:%ld K\n",max_memcount/1024);
    printf("mosquttio current malloc memory:%ld B\n",memcount);
    printf("mosquttio total free memory:%ld B\n",free_memcount);
    printf("mosquttio epoll register fd:%ld\n",g_epoll_register_fd);
    printf("mosquttio epoll unregiter fd:%ld\n",g_epoll_unregister_fd);
    printf("mosquttio epoll monitor fd:%ld\n",g_epoll_register_fd-g_epoll_unregister_fd);
    printf("mosquttio epoll expired client:%ld\n",g_epoll_expired_fd);
    printf("mosquttio epoll socket connections:%ld\n",g_epoll_sock_connections);
    printf("mosquttio epoll mqtt connections:%ld\n",g_epoll_mqtt_connections);
    printf("mosquttio epoll publish counts:%ld\n",g_epoll_publish_num);
    printf("mosquttio epoll subscribe counts:%ld\n",g_epoll_subscribe_num);
    printf("mosquttio epoll unsubscribe counts:%ld\n",g_epoll_unsubscribe_num);
    printf("===================================================\n");
}
...
int main(int argc, char *argv[])
{
    ...
    if(config.epoll)
    {
        _mosquitto_log_printf(NULL, MOSQ_LOG_INFO, "mosquitto mode is epoll.");
        rc = mosquitto_main_epoll_loop(&int_db, listensock, listensock_count, listener_max);
    }
    else
    {
        _mosquitto_log_printf(NULL, MOSQ_LOG_INFO, "mosquitto mode is poll.");
